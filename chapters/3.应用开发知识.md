...
### 3.1.2 如何在页面之间传递变量（至少两种)?
GET POST SESSION COOKIE NGINX配置定义
...
## 3.3 其它
### 3.3.1 如何实现静态化？
静态化就是将动态网页以静态HTML的形式展现给用户，分为伪静态化和纯静态化
伪静态化是利用NGINX或APACHE的URL重写功能，将URL变为.html后缀并响应，实现方式有
1. 通过file_get_contents()函数实现
2. OB函数
3. 模板
4. 参数替换+fopen读写

静态化的好处：
1.减少动态内容处理和IO读写，减轻服务器负担
2.有利于搜索引擎收录
3.提高加载速度
4.提高安全性
5.避免全站崩溃
